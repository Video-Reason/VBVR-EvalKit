...
class BallBounceEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific_interleave(
        ...
        if gen_ball is not None and gt_ball is not None:
            ...
            # Strict tolerance: ball must land within 20px of GT position
            scores['physics'] = max(0, 1.0 - dist / 20.0)
        else:
            scores['physics'] = 0.0
        ...
        if gt_bounces > 0:
            # Require exact bounce count match
            if pred_bounces == gt_bounces:
                scores['bounce_count'] = 1.0
            elif abs(pred_bounces - gt_bounces) == 1:
                scores['bounce_count'] = 0.3
            else:
