...
class GridShiftEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific_interleave(
        ...
        # Strict: both completeness and pattern must be high to proceed
        block_preserved = min(completeness_score, pattern_score) > 0.85
        scores['completeness'] = min(completeness_score, pattern_score)
...
class SlidingPuzzleEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific(
        ...
        target_correct = target_score > 0.8
    ...
    def _evaluate_target_state_rule_based(self, gen_frame: np.ndarray, gt_frame: np.ndarray) -> float:
        ...
        # Strict: puzzle arrangement must be near-exact match
        if diff < 5:
            return 1.0
        elif diff < 10:
            return 0.2
...
class TrafficLightEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific_interleave(
        ...
        # final_state_accuracy (50%): reuse — strict gate
        state_score = self._evaluate_final_state(last_frame, gt_last)
        scores['final_state_accuracy'] = state_score

        # opposite_rule (25%): reuse
        scores['opposite_rule'] = self._evaluate_opposite_rule(last_frame)

        # If final state is wrong, penalize diff_ssim heavily
        if state_score < 0.5:
            scores['diff_ssim'] = 0.0
        else:
            # diff_ssim (25%): replace countdown + switch_timing
            pred_diff = cv2.absdiff(last_frame, input_resized)
...
class CommunicatingVesselsEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific_interleave(
        ...
        # Strict: equilibrium is the core requirement — if it fails, cap all other scores
        if equilibrium_score < 0.7:
            scores['diff_ssim'] = 0.0
            scores['volume_conservation'] = 0.0
            scores['visual_fidelity'] = 0.0
            self._last_task_details = scores
            return sum(scores[k] * INTERLEAVE_WEIGHTS[k] for k in INTERLEAVE_WEIGHTS)
    ...
    def _evaluate_final_equilibrium_vs_gt(self, gen_levels: List[int], gt_levels: List[int]) -> float:
        ...
        # Strict: levels must be within 10px of each other AND match GT mean
        if gen_range <= 10:
