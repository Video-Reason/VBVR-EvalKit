...
class GridNumberSequenceEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific_interleave(
        ...
        if input_resized is not None:
            ...
            scores['path_optimal'] = compute_ssim(pred_path, gt_path) ** 2
        else:
            # Fallback: compare whole frames
            scores['path_optimal'] = compute_ssim(last_frame, gt_final_resized) ** 2
...
class GridGoThroughBlockEvaluator(BaseEvaluator):
    ...
    def _evaluate_task_specific_interleave(
        ...
        if input_resized is not None:
            pred_path = cv2.absdiff(last_frame, input_resized)
            gt_path = cv2.absdiff(gt_final_resized, input_resized)
            scores['path_ssim'] = compute_ssim(pred_path, gt_path) ** 2
        else:
            scores['path_ssim'] = compute_ssim(last_frame, gt_final_resized) ** 2
